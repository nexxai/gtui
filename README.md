...
A high-performance, terminal-based Gmail client built with Rust. Features secure OAuth2 authentication, local SQLite caching, background synchronization, and a vim-inspired interface.
...
4.  **Download JSON**: Save the client secret JSON file as `credentials.json` in the root of this project.
...
1.  **Build the project**:
    ...
    ```
2.  **Authenticate**:
    The first time you run the app, it will open your default web browser for Google authentication.
    ```bash
    cargo run
    ```
3.  **Permissions**:
    The app will request broad Gmail permissions to sync labels and send/read emails.
    The authentication token will be securely stored in your macOS Keychain via the `keyring` crate.

## Configuration

You can customize the application behavior by editing `settings.toml` in the project root.
...
Edit the `[keybindings]` section in `settings.toml`. You can provide multiple keys as an array of strings:
...
-   **Authentication Failed**: Ensure `credentials.json` is present and correctly configured in Google Cloud.
-   **Database Errors**: If the local cache becomes corrupted, you can safely delete `gtui.db` and the app will re-sync on next startup.
-   **Keychain Access**: Ensure the app has permission to access the macOS Keychain if prompted.
